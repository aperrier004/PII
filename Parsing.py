...
def toBase(filename):
    ...
    for d in data:
        ...
        temp_dict['PupilLeft'] = np.ones(len(temp_dict['Timestamp'])) * 7
        temp_dict['PupilRight'] = np.ones(len(temp_dict['Timestamp'])) * 7
        ...
        df = df.append(pd.DataFrame.from_dict(
            temp_dict, orient='index').transpose(), ignore_index=True, sort=False)
        del(temp_dict)
...
def convertToBase(filename, sensor_type, device, stim_list=None, start='START', stop=None, eye='B'):
    ...
    if sensor_type == 'EyeTracker':
        try:
            return toBase(filename)
        except Exception as e:
            print("Sorry " + sensor_type +
                  " data format not supported! The following exception was thrown: \n")
            print(e)
            return
...
def db_create(data_path, source_folder, database_name, dtype_dictionary=None, na_strings=None):
    ...
    for file in newlist:
        ...
        # To ensure table names dont haves spaces
        table_name = file_name_no_extension.replace(' ', '_')
        ...
        try:
            ...
            for value in conversion.name:
                ...
                if value == table_name:
                    ...
                    dummy = database.execute(query)
                    print(
                        "Dropped table as it existed previously and reinstantiated it: ", table_name)
                    break
        except ValueError:
        ...
        for df in pd.read_csv(file_name, chunksize=chunksize, iterator=True, na_values=na_strings, dtype=dtype_dictionary):
            ...
            df.index += j
            i += 1
            df.to_sql(table_name, database, if_exists='append',
    ...
    if os.path.isdir(exp_path):
        ...
        for f in os.listdir(data_path):
            ...
            if stim_list_mode == "diff":
                stim = np.loadtxt(data_path + "/stim/" +
                                  f.split(".")[0] + ".txt", dtype=str)

            df = convertToBase(data_path + "/" + f, sensor_type='EyeTracker',
                               device=device, stim_list=stim, start=start, stop=stop, eye=eye)
