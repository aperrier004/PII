...
class Stimulus:
    ...
    def smooth(self, x, window_len):
        ...
        for i in range(len(x)-window_len//2, len(x)):
            y[i] = np.sum(y[i - (len(x) - i - 1): i + (len(x) - i - 1)]
                          ) / ((2*(len(x) - i - 1)) + 1)
    ...
    def findBlinks(self, pupil_size, gaze=None, sampling_freq=1000, concat=False, concat_gap_interval=100, interpolate=False):
        ...
        smooth_pupil_size = np.array(self.smooth(
            pupil_size, samples2smooth), dtype='float32')
        ...
        if interpolate:
            ...
            pupil_size_no_blinks = np.hstack(
                (pupil_size_no_blinks, pupil_size[range(blink_offset[i], len(pupil_size))]))
            ...
            if gaze != None:
                ...
                for eye in ["left", "right"]:
                    ...
                    interp_gaze_x = np.interp(np.arange(len(pupil_size)), sorted(
                        pupil_size_no_blinks_indices), gaze_x_no_blinks)
                    interp_gaze_y = np.interp(np.arange(len(pupil_size)), sorted(
                        pupil_size_no_blinks_indices), gaze_y_no_blinks)
    ...
    def position2Velocity(self, gaze, sampling_freq):
        ...
        try:
            velocity[2:n-2] = (gaze[4:n] + gaze[3:n-1] -
                               gaze[1:n-3] - gaze[0:n-4]) * (sampling_freq/6.0)
    ...
    def findBinocularMS(self, msl, msr):
        ...
        if (numr * numl) > 0:
            ...
            for i in range(N):
                left = np.where((msl[:, 0] >= m[i, 0]) &
                                (msl[:, 1] <= m[i, 1]))[0]
                right = np.where((msr[:, 0] >= m[i, 0]) &
                                 (msr[:, 1] <= m[i, 1]))[0]
                ...
                if (len(right) * len(left)) > 0:
                    ...
                    if NB == 1:
                        ...
                        bin_ms = np.vstack(
                            (bin_ms, np.hstack((msl[left[il], :], msr[right[ir], :]))))
                else:
                    if len(right) == 0:
                        NL += 1
                        ampl = np.sqrt((msl[left, 5]**2 + msl[left, 6]**2))
        ...
        ms = {"NB": NB, "NR": NR, "NL": NL,
              "bin": bin_ms, "left": monol, "right": monor}
            ...
            if (ms_indices[k+1] - ms_indices[k]) == 1:
                ...
                if duration >= MINDUR:
                    ...
                    if num_ms == 1:
                        ...
                        new_ms = np.array(
                            [ms_indices[a], ms_indices[b], 0, 0, 0, 0, 0, 0, 0])
    ...
    def findMicrosaccades(self, sampling_freq=1000, plot_ms=False):
        ...
        for fix_ind in range(len(fixation_indices["start"])):
            ...
            for i in ["left", "right"]:
                ...
                smooth_gaze_x = self.smoothGaze(self.position2Velocity(
                    curr_gaze["x"], sampling_freq=1), curr_gaze["x"], sampling_freq)
                smooth_gaze_y = self.smoothGaze(self.position2Velocity(
                    curr_gaze["y"], sampling_freq=1), curr_gaze["y"], sampling_freq)
            ...
            if plot_ms and MS["NB"] != 0:
                ...
                # Plotting positions
                a1.plot(smooth_gaze["left"]["x"][1:],
                        smooth_gaze["left"]["y"][1:])
                a1.set_xlabel("x")
                a1.set_ylabel("y")
                a1.set_title("Gaze Plot	")
                for i in range(MS["NB"]):
                    a1.plot(smooth_gaze["left"]["x"][int(MS["bin"][i][0]): int(
                        MS["bin"][i][1]) + 1], smooth_gaze["left"]["y"][int(MS["bin"][i][0]): int(MS["bin"][i][1]) + 1], color='r')
                # a1.set_xlim([-0.35, 0.25])
                # a1.set_ylim([-0.2, 1.25])

                e = Ellipse((0, 0), 2*MS["bin"][0][7], 2*MS["bin"]
                            [0][8], linestyle='--', color='g', fill=False)
                a2.add_patch(e)
                ...
                for i in range(MS["NB"]):
                    a2.plot(vel["left"]["x"][int(MS["bin"][i][0]): int(MS["bin"][i][1]) + 1],
                            vel["left"]["y"][int(MS["bin"][i][0]): int(MS["bin"][i][1]) + 1], color='r')
                # a2.set_xlim([-25, 40])
                # a2.set_ylim([-65, 70])

                if not os.path.isdir(self.path + "/Subjects/" + self.subject_name + "/ms_gaze_vel/"):
                    os.makedirs(self.path + "/Subjects/" +
                                self.subject_name + "/ms_gaze_vel/")

                fig.savefig(self.path + "/Subjects/" + self.subject_name +
                            "/ms_gaze_vel/" + self.name + "_" + str(fix_ind) + ".png", dpi=200)
                plt.close(fig)
                ...
                for i in range(MS["NB"]):
                    ...
                    amp = (np.sqrt(MS["bin"][i][5]**2 + MS["bin"][i][6]**2) +
                           np.sqrt(MS["bin"][i][13]**2 + MS["bin"][i][14]**2))/2
                    ax.scatter(amp, peak_vel, marker='o',
                               facecolors='none', edgecolors='r')

        if plot_ms:
            fig2.savefig(self.path + "/Subjects/" + self.subject_name +
                         "/ms_main_seq" + self.name + ".png", dpi=200)
            plt.close(fig2)
            ...
            if ms["NB"] != 0:
                for m in ms["bin"]:
                    if len(np.where(self.data["GazeAOI"][int(m[0]): int(m[1])] == 1)[0]) > int(0.9 * (m[1] - m[0])):
                        ...
                        amp_val = (np.sqrt(m[5]**2 + m[6]**2) +
                                   np.sqrt(m[13]**2 + m[14]**2)) / 2.
    ...
    def findSaccadeParams(self, sampling_freq=1000):
        ...
        for start, end in zip(saccade_onset, saccade_offset):
            ...
            if len(np.where(self.data["GazeAOI"][start:end] == 1)[0]) > int(0.9 * (end-start)):
                ...
                vel_x = self.position2Velocity(
                    self.data["InterpGaze"]["left"]["x"][start:end], sampling_freq)
                vel_y = self.position2Velocity(
                    self.data["InterpGaze"]["left"]["y"][start:end], sampling_freq)
                ...
                ix1 = np.argmin(self.data["InterpGaze"]
                                ["left"]["x"][start:end])
                ix2 = np.argmax(self.data["InterpGaze"]
                                ["left"]["x"][start:end])
                iy1 = np.argmin(self.data["InterpGaze"]
                                ["left"]["y"][start:end])
                iy2 = np.argmax(self.data["InterpGaze"]
                                ["left"]["y"][start:end])
                dX = np.sign(ix2 - ix1) * (maxx - minx)
                dY = np.sign(iy2 - iy1) * (maxy - miny)
    ...
    def findFixationParams(self):
        ...
        fix_num, fix_ind, fix_cnt = np.unique(
            self.data["FixationSeq"], return_index=True, return_counts=True)
    ...
    def findPupilParams(self):
        ...
        pupil_size = self.data["InterpPupilSize"] - \
            self.data["InterpPupilSize"][0]
    ...
    def gazePlot(self, save_fig=False, show_fig=False, save_data=False):
        ...
        fixation_indices = np.vstack(
            (fixation_dict["start"], fixation_dict["end"]))
        fixation_indices = np.reshape(
            fixation_indices, (fixation_indices.shape[0] * fixation_indices.shape[1]), order='F')

        gaze_x = np.array(
            np.split(self.data["InterpGaze"]["left"]["x"], fixation_indices))
        gaze_y = np.array(
            np.split(self.data["InterpGaze"]["left"]["y"], fixation_indices))
        ...
        ax.plot(self.data["InterpGaze"]["left"]["x"],
                self.data["InterpGaze"]["left"]["y"], 'r-')

        i = 0
        exp_data = {"ind": [], "x": [], "y": [], "gaze_x": self.data["InterpGaze"]
                    ["left"]["x"], "gaze_y": self.data["InterpGaze"]["left"]["y"]}
        ...
        if save_fig:
            fig.savefig(self.path + "/Subjects/" + self.subject_name +
                        "/gaze_plot_" + self.name + ".png", dpi=300)

        if save_data:
            max_len = max([len(exp_data[x]) for x in exp_data])
            for key in exp_data:
                exp_data[key] = np.pad(
                    exp_data[key], (0, max_len - len(exp_data[key])), 'constant', constant_values=float('nan'))
            pd.DataFrame().from_dict(exp_data).to_csv(self.path + "/Subjects/" +
                                                      self.subject_name + "/gaze_plot_data_" + self.name + ".csv")
    ...
    def gazeHeatMap(self, save_fig=False, show_fig=False, save_data=False):
        ...
        hist, _, _ = np.histogram2d(y, x, bins=[int(col_shape*downsample_fraction), int(
            row_shape*downsample_fraction)], range=[[0, int(col_shape)], [0, int(row_shape)]])
        hist = gaussian_filter(hist, sigma=12)
        hist = hist.repeat(int(1/downsample_fraction),
                           axis=0).repeat(int(1/downsample_fraction), axis=1)
        ...
        ax.contourf(np.arange(0, int(row_shape), 1), np.arange(
            0, int(col_shape), 1), hist, cmap=my_cmap)
        ...
        if save_fig:
            fig.savefig(self.path + "/Subjects/" + self.subject_name +
                        "/gaze_heatmap_" + self.name + ".png", dpi=300)
    ...
    def visualize(self, show=False, save_data=False):
        ...
        if self.data["InterpGaze"] != None:
            ...
            # Plot for eye gaze
            line, = ax.plot(self.data["InterpGaze"]["left"]["x"][:1],
                            self.data["InterpGaze"]["left"]["y"][:1], 'r-', alpha=1)
            circle, = ax.plot(self.data["InterpGaze"]["left"]["x"][1],
                              self.data["InterpGaze"]["left"]["y"][1], 'go', markersize=10, alpha=0.7)
            ax.set_title("Gaze")

            # Plot for pupil size
            line3, = ax2.plot(
                total_range[:1], self.data["InterpPupilSize"][:1])
            ...
            for i in range(len(self.data["BlinksLeft"]["blink_onset"])):
                plt.axvline(x=self.data["BlinksLeft"]["blink_onset"]
                            [i], linestyle="--", color="r", alpha=0.4)
                plt.axvline(x=self.data["BlinksLeft"]["blink_offset"]
                            [i], linestyle="--", color="g", alpha=0.6)
        ...
        def update(i):
            ...
            if self.data["InterpGaze"] != None:
                ...
                ax2.set_ylim([min(self.data["InterpPupilSize"][:i]) - 5,
                              max(self.data["InterpPupilSize"][:i]) + 5])
    ...
    def findEyeMetaData(self):
        ...
        self.sensors["EyeTracker"].metadata["blink_rate"] = blink_cnt / \
            self.response_time
        ...
        # Saccade Features
        saccade_count, saccade_duration, saccade_peak_vel, saccade_amplitude = self.findSaccadeParams(
            self.sampling_freq)
    ...
    def getData(self, data, sensor_names):
        ...
        for col_class in sensor_names:
            if col_class == "EyeTracker":
                ...
                fixation_seq_df = np.array(
                    data.FixationSeq.fillna(-1), dtype='float32')
                fixation_seq = np.squeeze(
                    np.array([fixation_seq_df[i] for i in sorted(et_rows)], dtype="float32"))

                # Extracting the eye gaze data
                l_gaze_x = np.squeeze(
                    np.array([l_gazex_df[i] for i in sorted(et_rows)], dtype="float32"))
                l_gaze_y = np.squeeze(
                    np.array([l_gazey_df[i] for i in sorted(et_rows)], dtype="float32"))
                l_gaze = {"x": l_gaze_x, "y": l_gaze_y}
                r_gaze_x = np.squeeze(
                    np.array([r_gazex_df[i] for i in sorted(et_rows)], dtype="float32"))
                r_gaze_y = np.squeeze(
                    np.array([r_gazey_df[i] for i in sorted(et_rows)], dtype="float32"))
                r_gaze = {"x": r_gaze_x, "y": r_gaze_y}
                gaze = {"left": l_gaze, "right": r_gaze}

                # Extracting Pupil Size Data
                pupil_size_r = np.squeeze(
                    np.array([pupil_size_r_df[i] for i in sorted(et_rows)], dtype="float32"))
                pupil_size_l = np.squeeze(
                    np.array([pupil_size_l_df[i] for i in sorted(et_rows)], dtype="float32"))

                # Fixing Blinks and interpolating pupil size and gaze data
                blinks_l, interp_pupil_size_l, new_gaze_l = self.findBlinks(
                    pupil_size_l, gaze=gaze, sampling_freq=self.sampling_freq, interpolate=True, concat=True)
                blinks_r, interp_pupil_size_r, new_gaze_r = self.findBlinks(
                    pupil_size_r, gaze=gaze, sampling_freq=self.sampling_freq, interpolate=True, concat=True)
                interp_pupil_size = np.mean(
                    [interp_pupil_size_r, interp_pupil_size_l], axis=0)

                gaze_aoi = self.setAOICol(
                    [gaze_aoi_df, new_gaze_l["left"]["x"], new_gaze_l["left"]["y"]])
...
def groupHeatMap(sub_list, stim_name, json_file, save_fig=False):
    ...
    for sub in sub_list:
        if sub.stimulus[stim_type][stim_num].data != None:
            x = np.concatenate(
                (x, sub.stimulus[stim_type][stim_num].data["InterpGaze"]["left"]["x"]))
            y = np.concatenate(
                (y, sub.stimulus[stim_type][stim_num].data["InterpGaze"]["left"]["y"]))
        else:
            cnt += 1
    ...
    try:
        img = plt.imread(path + "/Stimuli/" +
                         sub_list[0].stimulus[stim_type][stim_num].name + ".jpg")
    except:
        try:
            img = plt.imread(
                path + "Stimuli/" + sub_list[0].stimulus[stim_type][stim_num].name + ".jpeg")
        except:
    ...
    if save_fig:
        ...
        fig.savefig(path + "/Aggregate_Plots" + "/agg_gaze_heatmap_" +
                    str(datetime.now().timestamp()).split(".")[0] + ".png", dpi=300)
